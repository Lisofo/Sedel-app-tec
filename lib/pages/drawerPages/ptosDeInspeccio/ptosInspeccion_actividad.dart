...
class _PtosInspeccionActividadState extends State<PtosInspeccionActividad> {
  ...
  late RevisionPtoInspeccion nuevaRevisionPtoInspeccion =
  ...
  cargarDatos() async {
    ...
    ptoInspeccionSeleccionados =
        context.read<OrdenProvider>().puntosSeleccionados;
    tareas = await TareasServices().getTareasXTPI(tPISeleccionado, modo, token);
    materiales =
        await MaterialesServices().getMaterialesXTPI(tPISeleccionado, token);
    ...
    for (var i = 0; i < ptoInspeccionSeleccionados.length; i++) {
      ...
      if (ptoInspeccionSeleccionados[i].otPuntoInspeccionId != 0) {
        await PtosInspeccionServices().putPtoInspeccionAccion(
            context, orden, ptoInspeccionSeleccionados[i], token);
      } else {
  ...
  Future<void> cargarLotes() async {
    lotesVencimientos = await MaterialesServices()
        .getLotes(materialSeleccionado!.materialId, token);
    setState(() {});
  }
